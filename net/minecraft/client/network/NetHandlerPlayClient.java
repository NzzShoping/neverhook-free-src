...
import ru.neverhook.event.impl.EventVelocity;
import ru.neverhook.feature.combat.Velocity;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handleEntityVelocity(SPacketEntityVelocity packetIn) {
        ...
        if (entity != null) {
            ...
            if (mode.equalsIgnoreCase("Matrix") && Main.instance.featureManager.getFeatureByClass(Velocity.class).getState()) {
                if (packetIn.getEntityID() == Minecraft.getMinecraft().player.getEntityId()) {
                    entity.setVelocity(0.0D, packetIn.getMotionY() / 8000.0D, packetIn.getMotionZ() / 8000.0D / 2.2D);
                    MovementUtil.strafe();
                }
            } else {
                EventVelocity velocity = new EventVelocity(entity, packetIn.getMotionX() / 8000.0D, packetIn.getMotionY() / 8000.0D, packetIn.getMotionZ() / 8000.0D);
                EventManager.call(velocity);
                if (!velocity.isCancelled()) {
                    entity.setVelocity(velocity.getMotionX(), velocity.getMotionY(), velocity.getMotionZ());
                }
            }
        }
    }
        ...
        if (entity != null) {
            if (packetIn.getOpCode() == 21) {
                ...
                if (entity == this.gameController.player) {
                    this.gameController.entityRenderer.renderTotem(new ItemStack(Items.TOTEM_OF_UNDYING));
                }
            } else {
