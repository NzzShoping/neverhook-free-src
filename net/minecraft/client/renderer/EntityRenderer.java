...
import ru.neverhook.event.impl.EventColorFov;
import ru.neverhook.feature.combat.KillAura;
import ru.neverhook.feature.combat.Reach;
import ru.neverhook.feature.player.NoEntityHit;
import ru.neverhook.feature.visual.CameraClip;
import ru.neverhook.feature.visual.NoRender;
import ru.neverhook.feature.world.FreeCam;
import ru.neverhook.feature.world.NoInteract;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            if (Main.instance.featureManager.getFeatureByClass(NoEntityHit.class).getState())
                return;

            for (Entity entity1 : list) {

                if (entity1 instanceof EntityArmorStand && Main.instance.featureManager.getFeatureByClass(NoInteract.class).getState() && NoInteract.armorStands.getValue())
                    return;

                if (!(Main.instance.featureManager.getFeatureByClass(KillAura.class).getState() && KillAura.target != null && KillAura.wellmore.getValue())) {
            ...
            if (this.pointedEntity != null) {
                if (Main.instance.friendManager.isFriend(this.pointedEntity.getName())) {
                    reach = 3.0D;
                } else {
                    reach = Main.instance.featureManager.getFeatureByClass(Reach.class).getState() ? Reach.expand.getValFloat() : 3.0D;
                }
            }
            if (this.pointedEntity != null && flag && vec3d.distanceTo(vec3d3) > reach) {
                this.pointedEntity = null;
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (Main.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.hurt.getValue())
            return;
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
            ...
            if (entitylivingbase.getHealth() <= 0.0F) {
                ...
                GlStateManager.rotate(380.0F - 8000.0F / (f1 + 200.0F), 0.0F, 0.0F, 1.0F);
            }
            ...
            GlStateManager.rotate(-f2, 0, 1.0F, 0.0F);
    ...
    public void orientCamera(float partialTicks) {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            animationFOV += (1 * Minecraft.frameTime) / 200;
            if (animationFOV >= thirdPersonDistancePrev) {
                animationFOV = thirdPersonDistancePrev;
            }
            double d3 = Main.instance.featureManager.getFeatureByClass(CameraClip.class).getState() ? CameraClip.fovModifier.getValInt() : this.animationFOV + (animationFOV - animationFOV) * partialTicks;
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    f4 = f4 * 0.1F;
                    f5 = f5 * 0.1F;
                    if (!Main.instance.featureManager.getFeatureByClass(CameraClip.class).getState()) {
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        if (flag) {
            ShadersRender.setupTerrain(renderglobal, entity, partialTicks, icamera, this.frameCount++, this.mc.player.isSpectator() && Main.instance.featureManager.getFeatureByClass(FreeCam.class).getState());
        } else {
            renderglobal.setupTerrain(entity, partialTicks, icamera, this.frameCount++, (this.mc.player.isSpectator() && Main.instance.featureManager.getFeatureByClass(FreeCam.class).getState()));
        }
    ...
    private void updateFogColor(float partialTicks) {
        ...
        EventColorFov event = new EventColorFov(this, entity, iblockstate1, partialTicks, this.fogColorRed, this.fogColorGreen, this.fogColorBlue);
        EventManager.call(event);
        ...
        GlStateManager.clearColor(fogColorRed, fogColorGreen, fogColorBlue, 0);
    ...
    private void setupFog(int startCoords, float partialTicks) {
        ...
        if (f >= 0.0F) {
            ...
            if (Main.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.liquidOverlay.getValue()) {
    ...
    public void renderTotem(ItemStack totem) {
        if (!Main.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.totem.getValue()) {
            this.field_190566_ab = totem;
