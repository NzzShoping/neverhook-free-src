...
import ru.neverhook.feature.visual.Animations;
...
public class ItemRenderer {
    ...
    private void renderArmFirstPerson(float p_187456_1_, float p_187456_2_, EnumHandSide p_187456_3_) {
        ...
        if (Main.instance.featureManager.getFeatureByClass(Chams.class).getState()) {
            GL11.glPushMatrix();
            GL11.glEnable(10754);
            ...
            if (!Chams.clientColor.getValue()) {
                GL11.glColor4f(color.darker().getRed() / 255F, color.darker().darker().getGreen() / 255F, color.darker().getBlue() / 255F, Chams.chamsAlpha.getValFloat());
            } else {
                GL11.glColor4f(Main.getClientColor().darker().getRed() / 255F, Main.getClientColor().darker().getGreen() / 255F, Main.getClientColor().darker().getBlue() / 255F, Chams.chamsAlpha.getValFloat());
            }
            GL11.glDisable(2929);
    ...
    private void transformFirstPerson(EnumHandSide p_187453_1_, float p_187453_2_) {
        ...
        if (Main.instance.featureManager.getFeatureByClass(Animations.class).getState() && Animations.item360.getValue()) {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_)) {
            ...
            if (p_187457_6_.func_190926_b()) {
                ...
                if (p_187457_1_.isHandActive() && p_187457_1_.getItemInUseCount() > 0 && p_187457_1_.getActiveHand() == p_187457_4_) {
                    ...
                    if (Animations.autoblock.getValue() && Main.instance.featureManager.getFeatureByClass(KillAura.class).getState() && KillAura.target != null && mc.player.getDistanceToEntity(KillAura.target) <= KillAura.range.getValFloat() && Main.instance.featureManager.getFeatureByClass(Animations.class).getState()) {
                        if (Animations.autoblock.getValue() && Main.instance.featureManager.getFeatureByClass(KillAura.class).getState() && KillAura.target != null && mc.player.getDistanceToEntity(KillAura.target) <= KillAura.range.getValFloat() && Main.instance.featureManager.getFeatureByClass(Animations.class).getState()) {
    ...
    private void renderFireInFirstPerson() {
        if (Main.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.fire.getValue()) {
