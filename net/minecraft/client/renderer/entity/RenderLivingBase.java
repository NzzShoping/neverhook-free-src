...
import ru.neverhook.feature.visual.Chams;
import ru.neverhook.feature.visual.ESP;
import ru.neverhook.feature.visual.NameTags;
...
public abstract class RenderLivingBase<T extends EntityLivingBase> extends Render<T> {
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
                || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, Float.valueOf(partialTicks), Double.valueOf(x), Double.valueOf(y), Double.valueOf(z))) {
            ...
            try {
                float f7, f = interpolateRotation(entity.prevRenderYawOffset, entity.renderYawOffset, partialTicks);
                ...
                if (!entity.isRiding()) {
                    if (Main.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.walk.getValue()) {
            ...
            if (Reflector.RenderLivingEvent_Post_Constructor.exists()) {
                Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Post_Constructor, entity, this, Float.valueOf(partialTicks), Double.valueOf(x), Double.valueOf(y), Double.valueOf(z));
            }
        }
    }
    ...
    protected void renderModel(T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor) {
        ...
        boolean chamsCheck = (Main.instance.featureManager.getFeatureByClass(Chams.class).getState() && entitylivingbaseIn instanceof EntityPlayer);
        ...
        if (flag || flag1) {
            ...
            if (chamsCheck) {
                ...
                if (!Chams.clientColor.getValue()) {
                    GlStateManager.color(color.darker().getRed() / 255F, color.darker().darker().getGreen() / 255F, color.darker().getBlue() / 255F, Chams.chamsAlpha.getValFloat());
                } else {
                    GlStateManager.color(Main.getClientColor().darker().getRed() / 255F, Main.getClientColor().darker().getGreen() / 255F, Main.getClientColor().darker().getBlue() / 255F, Chams.chamsAlpha.getValFloat());
                    ;
                }
            ...
            if (chamsCheck) {
                ...
                if (!Chams.clientColor.getValue()) {
                    GlStateManager.color(color.getRed() / 255F, color.getGreen() / 255F, color.getBlue() / 255F, Chams.chamsAlpha.getValFloat());
                } else {
                    GlStateManager.color(Main.getClientColor().getRed() / 255F, Main.getClientColor().getGreen() / 255F, Main.getClientColor().getBlue() / 255F, Chams.chamsAlpha.getValFloat());
                }
                this.mainModel.render(entitylivingbaseIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scaleFactor);
    ...
    public void renderName(T entity, double x, double y, double z) {
        if (Main.instance.featureManager.getFeatureByClass(NameTags.class).getState() || (Main.instance.featureManager.getFeatureByClass(ESP.class).getState())) {
            return;
        }
