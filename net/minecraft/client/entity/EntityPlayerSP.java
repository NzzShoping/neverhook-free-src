...
import ru.neverhook.feature.movement.NoSlowDown;
import ru.neverhook.feature.player.AntiPush;
import ru.neverhook.feature.player.NoClip;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdate() {
        if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            if (this.isRiding()) {
                ...
                if (!Main.instance.featureManager.getFeatureByClass(FreeCam.class).getState()) {
    ...
    private void onUpdateWalkingPlayer() {
        ...
        EventPostMotionUpdate eventPost = new EventPostMotionUpdate(this.rotationYaw, this.rotationPitch);
        ...
        if (this.isCurrentViewEntity()) {
            float lastReportedYaw = RotationUtil.Rotation.fakeYaw;
            float lastReportedPitch = RotationUtil.Rotation.fakePitch;
            AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
            double d0 = this.posX - this.lastReportedPosX;
            double d1 = axisalignedbb.minY - this.lastReportedPosY;
            double d2 = this.posZ - this.lastReportedPosZ;
            double d3 = this.rotationYaw - lastReportedYaw;
            double d4 = this.rotationPitch - lastReportedPitch;
            ...
            if (!Main.instance.featureManager.getFeatureByClass(FreeCam.class).getState()) {
                if (this.isRiding()) {
                    ...
                    this.connection.sendPacket(new CPacketPlayer.PositionRotation(this.posX, axisalignedbb.minY, this.posZ, this.rotationYaw, this.rotationPitch, ground));
                } else if (flag2) {
                    this.connection.sendPacket(new CPacketPlayer.Position(this.posX, axisalignedbb.minY, this.posZ, ground));
                } else if (flag3) {
                    this.connection.sendPacket(new CPacketPlayer.Rotation(this.rotationYaw, this.rotationPitch, ground));
            ...
            if (flag3) {
                this.lastReportedYaw = RotationUtil.Rotation.fakeYaw;
                this.lastReportedPitch = RotationUtil.Rotation.fakePitch;
            }
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        ...
        EventManager.call(pushOutBlock);
        if (pushOutBlock.isCancelled())
            return false;

        if ((Main.instance.featureManager.getFeatureByClass(NoClip.class)).getState() || (Main.instance.featureManager.getFeatureByClass(AntiPush.class).getState() && AntiPush.blocks.getValue()) || Main.instance.featureManager.getFeatureByClass(FreeCam.class).getState())
            return false;
        if (this.noClip) {
            return false;
        } else {
    ...
    public void onLivingUpdate() {
        ...
        if (isHandActive() && !isRiding()) {
            MovementInput.moveStrafe *= Main.instance.featureManager.getFeatureByClass(NoSlowDown.class).getState() ? NoSlowDown.percentage.getValFloat() / 100 : 0.2F;
            MovementInput.moveForward *= Main.instance.featureManager.getFeatureByClass(NoSlowDown.class).getState() ? NoSlowDown.percentage.getValFloat() / 100 : 0.2F;
        ...
        if (this.onGround && !flag1 && !flag2 && MovementInput.moveForward >= 0.8F && !this.isSprinting() && flag4 && !this.isHandActive() && !this.isPotionActive(MobEffects.BLINDNESS)) {
        ...
        if (!this.isSprinting() && MovementInput.moveForward >= 0.8F && flag4 && !this.isHandActive() && !this.isPotionActive(MobEffects.BLINDNESS) && this.mc.gameSettings.keyBindSprint.isKeyDown()) {
            this.setSprinting(true);
        }

        if (this.isSprinting() && (MovementInput.moveForward < 0.8F || this.isCollidedHorizontally || !flag4)) {
