...
import ru.neverhook.event.impl.EventRenderGui;
...
import ru.neverhook.feature.visual.ScoreBoard;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        EventRenderGui event = new EventRenderGui(scaledresolution, partialTicks);
        ...
        Event2D event1 = new Event2D(scaledresolution);
        ...
        if (scoreobjective1 != null) {
            ...
            EventManager.call(e);
            if (!Main.instance.featureManager.getFeatureByClass(ScoreBoard.class).getState() || !Main.instance.setmgr.getSettingByName("NoScoreBoard").getValue()) {
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score score1 : collection) {
            ...
            String neverhook = "vk.com/neverhook";
            int k = j1 - j * this.getFontRenderer().FONT_HEIGHT;
            int l = scaledRes.getScaledWidth() - 3 + 2;
            RenderUtil.drawRect(l1 - 2, k, l, k + this.getFontRenderer().FONT_HEIGHT, 0x60000000);
            mc.fontRendererObj.drawStringWithShadow(s1, l1, k, 553648127);
            if (j == collection.size()) {
                String s3 = objective.getDisplayName();
                RenderUtil.drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - 1, 0x60000000);
                RenderUtil.drawRect(l1 - 2, k - 1, l, k, 0x60000000);
                RenderUtil.drawGradientSideways(l1 - 2, k - 12, l, k + -10, Main.getClientColor().getRGB(), ColorUtils.rainbowNew(300, 1, 1));
                mc.fontRenderer.drawStringWithShadow(neverhook, l1 + i / 2 - mc.fontRenderer.getStringWidth(neverhook) / 2, k - mc.fontRenderer.getHeight(), ColorUtils.getGradientOffset(Color.RED, Main.getClientColor(), 3).getRGB());
            }
        }
    }
    ...
    private void renderPumpkinOverlay(ScaledResolution scaledRes) {
        if (Main.instance.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.pumpkin.getValue()) {
            return;
        }
        GlStateManager.disableDepth();
